...
Copy rules that survived the earlier de-slop pass: no heading that is two clipped sentences ending in full stops; the "free / no ticket" fact appears about three times, not thirteen; each initiation line says what the participant actually does.

**Biographies: none exists for Bawayi or Awa — do not write one.** ⚠️ **Joseph is the exception** (corrected 2026-08-10): he is **Joseph Saidi Simako**, a Shoonya teacher with a real sourced entry at `Event Submission/docs/teacher-data.md §22` — dancer and choreographer from the DR Congo, teaches **African Dance, Congolese dance and Congolese rumba** here, and has performed with Staff de Leader, Vibrations Schengen, Groupe Kabola and Groupe Furaya. Photo (1506×1684): `_Assets/shoonyadance/teacher-photos/joseph-saidi-simako/main.png`. The About section on all three language pages names him in full and says what he teaches — that Shoonya connection is the event's strongest local hook, so keep it.
